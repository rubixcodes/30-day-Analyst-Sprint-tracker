...
const MILESTONES = [
  ...
  { id: 'proj1',    icon: '🗂️', name: 'First Project Done',     desc: 'Complete your first portfolio project', check: s => s.projects.some(p => p.status === 'Completed') },
...
let state = {
  ...
  tasks:        {},      // { 'day-N': { taskId: bool, 'job1': 'Company - Role', ... } }
  applications: [],
  courses:      [],
  posts:        [],
  projects:     [],
...
function loadState() {
  try {
    ...
    if (state.settings.visaExpiry) document.getElementById('visaExpiry').value = state.settings.visaExpiry;
...
function initPosts() {
  if (!state.posts || state.posts.length !== 8) {
    state.posts = LI_POST_DAYS.map((day, i) => ({
      id: i + 1,
      day,
      topic: LI_POST_TOPICS[i] || '',
      status: 'Draft',
      datePub: '',
      views: '', likes: '', comments: '', newConn: '',
      link: ''
    }));
    saveState();
  }
...
function updateSettings() {
  ...
  state.settings.visaExpiry = document.getElementById('visaExpiry').value;
  saveState();
  renderDashboard();
}
...
function renderDashboard() {
  ...
  if (state.settings.startDate) {
    ...
    document.getElementById('dayCounter').textContent = clamped <= 30 ? `${clamped} / 30` : '30 / 30';
    const daysLeft = Math.max(0, 30 - dayNum + 1);
    document.getElementById('sprintLeft').textContent = daysLeft;

    // Overall progress by day
  ...
  if (state.settings.visaExpiry) {
    ...
    const el = document.getElementById('visaLeft');
    el.textContent = vDays > 0 ? vDays : 'Expired';
    el.style.color = vDays < 30 ? 'var(--danger)' : vDays < 90 ? 'var(--warning)' : 'var(--warning)';
  }

  // Completed days
  const completedCount = Object.keys(state.tasks).filter(k => {
    const day = state.tasks[k];
    return isDayComplete(parseInt(k.replace('day-','')));
  }).length;
  ...
  document.getElementById('appCount').textContent = appTotal;
  setStatus('appStatusPill', appTotal, 150);
  ...
  document.getElementById('ibmPct').textContent = ibmPct + '%';
  document.getElementById('ibmStatusPill').textContent = ibmPct >= 50 ? 'AHEAD' : ibmPct >= 30 ? 'ON TRACK' : 'BEHIND';
  document.getElementById('ibmStatusPill').className = 'status-pill ' + (ibmPct >= 50 ? 'ahead' : ibmPct >= 30 ? '' : 'behind');
  // IBM overall bar
  document.getElementById('ibmOverallPct').textContent = ibmPct + '%';
  document.getElementById('ibmOverallBar').style.width = ibmPct + '%';

  // LinkedIn
  const liPublished = state.posts.filter(p => p.status === 'Published').length;
  setRing('liRing', liPublished, 8);
  document.getElementById('liCount').textContent = liPublished;
  setStatus('liStatusPill', liPublished, 8);

  // Portfolio
  const projDone = state.projects.filter(p => p.status === 'Completed').length;
  setRing('portRing', projDone, 3);
  document.getElementById('portCount').textContent = projDone;
  setStatus('portStatusPill', projDone, 3);

  // Interviews
  const intCount = state.applications.filter(a => ['Interview Scheduled','Interview Done'].includes(a.status)).length;
  setRing('intRing', intCount, 10);
  document.getElementById('intCount').textContent = intCount;
  document.getElementById('intStatusPill').textContent = intCount >= 5 ? 'AHEAD' : intCount >= 2 ? 'ON TRACK' : 'BEHIND';
  document.getElementById('intStatusPill').className = 'status-pill ' + (intCount >= 5 ? 'ahead' : intCount >= 2 ? '' : 'behind');
}
...
function setStatus(pillId, val, max) {
  const pct = val / max;
...
function initDaySelector() {
  ...
  // Default to today's sprint day if set
  if (state.settings.startDate) {
    const start = new Date(state.settings.startDate); start.setHours(0,0,0,0);
    const today = new Date(); today.setHours(0,0,0,0);
    const day = Math.max(1, Math.min(30, Math.floor((today - start) / 86400000) + 1));
    sel.value = day;
  }
}
...
function buildDayTaskDefs(day) {
  ...
  return {
    ...
    afternoon: {
      label: '☀️ Afternoon Block — 3hrs · IBM BI Study',
      tasks: [
...
function getAllTaskIds(day) {
  ...
  Object.values(defs).forEach(block => {
    block.tasks.forEach(t => {
      if (t.type === 'job') ids.push(t.id + '_done');
      else ids.push(t.id);
    });
  });
  return ids;
...
function renderDayTasks() {
  const day = parseInt(document.getElementById('taskDaySelect').value);
  const key = `day-${day}`;
  ...
  ['morning','afternoon','evening'].forEach(blockKey => {
    ...
    html += `<div class="task-block">
      <div class="block-header"><span class="block-ico"></span>${block.label}</div>`;
      ...
      if (task.type === 'job') {
        ...
        html += `<div class="task-item">
          ...
          <div class="task-input-wrap">
            ...
            <input type="text" class="task-job-input ${filled}" placeholder="Company — Role — Platform"
              value="${val}"
...
function setTask(key, taskId, value) {
  ...
  saveState();
  // Refresh completion badge without full re-render
  ...
  if (complete && !badgeEl) {
    ...
    if (header) header.insertAdjacentHTML('beforeend', '<div class="day-complete-badge">✅ Day Complete!</div>');
...
function openAppModal(editIndex = null) {
  document.getElementById('appEditIndex').value = editIndex !== null ? editIndex : '';
  document.getElementById('appModalTitle').textContent = editIndex !== null ? 'Edit Application' : 'Add Application';

  if (editIndex !== null) {
    const a = state.applications[editIndex];
    document.getElementById('appDate').value    = a.date    || '';
    document.getElementById('appCompany').value = a.company || '';
    document.getElementById('appRole').value    = a.role    || '';
    document.getElementById('appSalary').value  = a.salary  || '';
    document.getElementById('appVisa').value    = a.visa    || 'Y';
    document.getElementById('appIndustry').value= a.industry|| 'Logistics';
    document.getElementById('appPlatform').value= a.platform|| 'LinkedIn';
    document.getElementById('appCL').value      = a.cl      || 'Y';
    document.getElementById('appStatus').value  = a.status  || 'Applied';
    document.getElementById('appFollowup').value= a.followup|| '';
    ...
    document.getElementById('appSalary').value  = '';
    document.getElementById('appVisa').value    = 'Unknown';
    document.getElementById('appIndustry').value= 'Logistics';
    document.getElementById('appPlatform').value= 'LinkedIn';
    document.getElementById('appCL').value      = 'N';
    document.getElementById('appStatus').value  = 'Applied';
    document.getElementById('appFollowup').value= '';
...
function saveApplication() {
  ...
  const app = {
    ...
    visa:     document.getElementById('appVisa').value,
    industry: document.getElementById('appIndustry').value,
    platform: document.getElementById('appPlatform').value,
    cl:       document.getElementById('appCL').value,
  ...
  if (idx !== '') {
    state.applications[parseInt(idx)] = app;
  } else {
...
function renderAppsTable() {
  ...
  const industryF = document.getElementById('filterIndustry').value;
  const platformF = document.getElementById('filterPlatform').value;
  ...
  document.getElementById('totalApps').textContent       = all.length;
  document.getElementById('totalResponses').textContent  = all.filter(a => ['Response','Interview Scheduled','Interview Done','Offer'].includes(a.status)).length;
  document.getElementById('totalInterviews').textContent = all.filter(a => ['Interview Scheduled','Interview Done'].includes(a.status)).length;
  document.getElementById('totalOffers').textContent     = all.filter(a => a.status === 'Offer').length;
  document.getElementById('totalRejected').textContent   = all.filter(a => a.status === 'Rejected').length;
  ...
  const statusBadge = s => {
    ...
    return `<span class="status-badge ${map[s]||'badge-applied'}">${s||'Applied'}</span>`;
  };
  ...
  tbody.innerHTML = apps.map((a, visIdx) => `
    <tr>
      <td>${a._idx + 1}</td>
      <td>${a.date || '—'}</td>
...
function exportAppsCSV() {
  ...
  const rows = state.applications.map((a, i) => [
    i+1, a.date, a.company, a.role, a.salary, a.visa, a.industry, a.platform, a.cl, a.status, a.followup, a.notes
  ]);
...
function renderCourses() {
  ...
  grid.innerHTML = state.courses.map((c, i) => {
    ...
    return `
    <div class="course-card ${cls}" id="course-card-${i}">
      ...
      <div class="course-fields">
        <div class="field-small">
          ...
          <input type="date" value="${c.startDate||''}" onchange="updateCourse(${i},'startDate',this.value)">
        </div>
        <div class="field-small">
          <label>Completion Date</label>
          <input type="date" value="${c.completionDate||''}" onchange="updateCourse(${i},'completionDate',this.value)">
        </div>
        <div class="field-small">
          <label>Hours Spent</label>
          <input type="number" min="0" max="100" value="${c.hours||''}" placeholder="0"
            onchange="updateCourse(${i},'hours',this.value)">
        </div>
        <div class="field-small">
          <label>Confidence (${c.confidence||5}/10)</label>
          <input type="range" class="confidence-slider" min="1" max="10" value="${c.confidence||5}"
            oninput="updateCourse(${i},'confidence',this.value); this.previousElementSibling.textContent='Confidence ('+this.value+'/10)'">
        </div>
        <div class="field-small course-full">
          <label>Key Skills Learned</label>
          <input type="text" value="${esc(c.skills||'')}" placeholder="e.g. pivot tables, DAX, ETL..."
            onchange="updateCourse(${i},'skills',this.value)">
        </div>
        <div class="field-small course-full">
          <label>Notes</label>
          <textarea rows="2" placeholder="Notes..." onchange="updateCourse(${i},'notes',this.value)">${esc(c.notes||'')}</textarea>
        </div>
      </div>
    </div>`;
  }).join('');
...
function updateCourse(idx, field, value) {
  state.courses[idx][field] = field === 'confidence' ? parseInt(value) : value;
  // Update card class
    ...
    <div class="post-card ${cls}" id="post-card-${i}">
      ...
      <div class="post-metrics">
        <div class="post-metric">
          <label>👁 Views</label>
          <input type="number" value="${p.views||''}" placeholder="0"
            onchange="updatePost(${i},'views',this.value)">
        </div>
        <div class="post-metric">
          <label>❤️ Likes</label>
          <input type="number" value="${p.likes||''}" placeholder="0"
            onchange="updatePost(${i},'likes',this.value)">
        </div>
        <div class="post-metric">
          <label>💬 Comments</label>
          <input type="number" value="${p.comments||''}" placeholder="0"
            onchange="updatePost(${i},'comments',this.value)">
        </div>
        <div class="post-metric">
          <label>🤝 Conns</label>
          <input type="number" value="${p.newConn||''}" placeholder="0"
            onchange="updatePost(${i},'newConn',this.value)">
        </div>
      </div>
      <div class="field-small" style="margin-bottom:6px">
        <label style="font-size:10px;color:var(--muted);display:block;margin-bottom:3px">Date Published</label>
        <input type="date" class="post-topic-input" value="${p.datePub||''}"
          onchange="updatePost(${i},'datePub',this.value)" style="margin-bottom:0">
      </div>
      <input type="url" class="post-link-input" placeholder="LinkedIn post URL..."
        value="${esc(p.link||'')}"
        onchange="updatePost(${i},'link',this.value)">
    </div>`;
  }).join('');
}
...
function updatePost(idx, field, value) {
  ...
  const card = document.getElementById(`post-card-${idx}`);
...
function renderProjects() {
  ...
  grid.innerHTML = state.projects.map((p, i) => {
    const donePct = p.subtasks.length
      ? Math.round((p.subtasks.filter(t => t.done).length / p.subtasks.length) * 100)
      : 0;
    ...
    const subtaskHtml = p.subtasks.map((t, si) => `
      <div class="subtask-item">
        ...
        <label for="st-${i}-${si}">${esc(t.label)}</label>
      </div>
    `).join('');

    return `
    <div class="proj-card ${cls}" id="proj-card-${i}">
      ...
      <div class="proj-body">
        ...
        <div class="proj-dates">
          ...
          <div class="proj-field">
            <label>Completion Date</label>
            <input type="date" value="${p.completionDate||''}" onchange="updateProject(${i},'completionDate',this.value)">
          </div>
        </div>
        ...
        <div class="proj-progress-bar">
          ...
          <div class="progress-track"><div class="progress-fill" style="width:${donePct}%"></div></div>
        </div>
        <div class="proj-link-wrap">
          <label>LinkedIn Post URL (when published)</label>
          <input type="url" class="proj-link-input" value="${esc(p.liLink||'')}" placeholder="https://linkedin.com/..."
            onchange="updateProject(${i},'liLink',this.value)">
        </div>
      </div>
    </div>`;
  }).join('');
  ...
  document.getElementById('weeklyContainer').innerHTML = `
    <div class="weekly-form">
      ...
      <div class="weekly-text-fields">
        ...
        ${wfText(w,'challenges','⚠️ Challenges',wr.challenges,3)}
      ...
      <div class="weekly-confidence">
        <div style="font-size:11px;color:var(--muted);margin-bottom:6px;display:flex;justify-content:space-between">
          ...
          <span class="accent-text" id="conf-label-${w}">${wr.confidence||5}/10</span>
        </div>
        <input type="range" class="confidence-slider" min="1" max="10" value="${wr.confidence||5}"
          oninput="setWeeklyField(${w},'confidence',parseInt(this.value));document.getElementById('conf-label-${w}').textContent=this.value+'/10'">
      </div>
      ...
      <div class="wf-field" style="margin-bottom:16px">
        <label>Portfolio Progress Notes</label>
        <textarea class="wf-textarea" rows="2" placeholder="What did you build this week?"
          onchange="setWeeklyField(${w},'portProgress',this.value)">${esc(wr.portProgress||'')}</textarea>
      </div>
...
function wfNum(w, field, label, val) {
  return `<div class="wf-field">
    ...
    <input type="number" class="wf-input" min="0" value="${val||''}" placeholder="0"
      onchange="setWeeklyField(${w},'${field}',parseInt(this.value)||0)">
  </div>`;
}
...
function wfText(w, field, label, val, rows) {
  return `<div class="wf-field">
    ...
    <textarea class="wf-textarea" rows="${rows}" placeholder="..."
      onchange="setWeeklyField(${w},'${field}',this.value)">${esc(val||'')}</textarea>
  </div>`;
}
...
function openNetworkModal(editIdx = null) {
  document.getElementById('netEditIndex').value = editIdx !== null ? editIdx : '';
  if (editIdx !== null) {
    const c = state.networking[editIdx];
    document.getElementById('netName').value     = c.name     || '';
    document.getElementById('netCompany').value  = c.company  || '';
    document.getElementById('netRole').value     = c.role     || '';
    document.getElementById('netPlatform').value = c.platform || 'LinkedIn';
    document.getElementById('netDate').value     = c.date     || '';
    document.getElementById('netMsg').value      = c.msg      || 'Y';
    document.getElementById('netReply').value    = c.reply    || 'N';
    document.getElementById('netFollowup').value = c.followup || 'Y';
    document.getElementById('netNotes').value    = c.notes    || '';
  } else {
    document.getElementById('netName').value     = '';
    document.getElementById('netCompany').value  = '';
    document.getElementById('netRole').value     = '';
    document.getElementById('netPlatform').value = 'LinkedIn';
    document.getElementById('netDate').value     = new Date().toISOString().split('T')[0];
    document.getElementById('netMsg').value      = 'Y';
    document.getElementById('netReply').value    = 'N';
    document.getElementById('netFollowup').value = 'Y';
    document.getElementById('netNotes').value    = '';
  }
  openModal('networkModal');
}

function saveContact() {
  const idx = document.getElementById('netEditIndex').value;
  const contact = {
    name:     document.getElementById('netName').value.trim(),
    company:  document.getElementById('netCompany').value.trim(),
    role:     document.getElementById('netRole').value.trim(),
    platform: document.getElementById('netPlatform').value,
    date:     document.getElementById('netDate').value,
    msg:      document.getElementById('netMsg').value,
    reply:    document.getElementById('netReply').value,
    followup: document.getElementById('netFollowup').value,
    notes:    document.getElementById('netNotes').value.trim()
  };
  if (!contact.name) { showToast('Name is required', 'error'); return; }

  if (idx !== '') state.networking[parseInt(idx)] = contact;
...
function checkMilestones() {
  ...
  if (anyNew) { saveState(); renderMilestones(); }
}
...
function exportJSON() {
  ...
  downloadFile('sprint30-backup.json', data, 'application/json');
